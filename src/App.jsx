...
import { BarChart3, TrendingUp, Users, AlertTriangle, CheckCircle, XCircle, Clock, MapPin, TrendingDown, Home, Upload, FileJson, Download, Calendar, BarChart, FileText, Menu, PieChart, CloudDownload, Trash2, AlertCircle } from 'lucide-react';

const PSMMonitorApp = () => {
  console.log('🚀 PSM Monitor v3.49.10 - CORRIGIDO: naoTestadas agora salva no estado');
        ...
        <div className="p-4">
          <nav className="space-y-1">
            ...
            <label className="w-full flex items-center space-x-3 px-4 py-3 text-gray-700 hover:bg-yellow-50 rounded-lg transition-colors cursor-pointer border-2 border-yellow-500">
              <CloudDownload className="w-5 h-5 text-yellow-600" />
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">v3.40.9 - Scroll Inteligente UNIFICADO! ✅🎯</p>
                </div>
              </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200">
                      <div className="flex items-center justify-between p-3 bg-gradient-to-r from-amber-50 to-yellow-50 rounded-lg border border-amber-200">
                        <div className="flex items-center space-x-2">
                          ...
                          <div>
                            <div className="text-sm font-bold text-gray-800">Índice de Excelência</div>
                            <div className="text-xs text-gray-600">Média ponderada de performance</div>
                          </div>
                        </div>
                        <div className="text-3xl font-bold text-amber-600">
                          {testesData.testadas > 0 ? (
                            (
                              ((testesData.semIndisponibilidade / testesData.testadas) * 0.3) +
                              ((testesData.concluidas / testesData.testadas) * 0.25) +
                              ((testesData.comGanho / testesData.testadas) * 0.25) +
                              ((testesData.estaveis / testesData.testadas) * 0.2)
                            ) * 100
                          ).toFixed(0) : 0}%
                        </div>
                      </div>
...
- Índice de Excelência: ${testesData.testadas > 0 ? ((
  ((testesData.semIndisponibilidade/testesData.testadas)*0.3)+
  ((testesData.concluidas/testesData.testadas)*0.25)+
  ((testesData.comGanho/testesData.testadas)*0.25)+
  ((testesData.estaveis/testesData.testadas)*0.2)
...
${Object.keys(todosTestesData).filter(psm => todosTestesData[psm]).map(psm => {
  ...
  return `
${psm}:
- Cadastradas: ${dados.cadastradas} | Testadas: ${dados.testadas} | Pendentes: ${dados.pendentes}
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {[
                          ...
                          { label: 'Taxa de Conclusão', key: 'taxaConclusao', format: (v) => `${v.toFixed(1)}%`, isPercentage: true },
                          { label: 'Taxa de Melhoria', key: 'taxaMelhoria', format: (v) => `${v.toFixed(1)}%`, isPercentage: true },
                          { label: 'Índice de Excelência', key: 'indiceExcelencia', format: (v) => `${v.toFixed(1)}%`, isPercentage: true, highlight: true }
                        ].map((metrica, idx) => (
