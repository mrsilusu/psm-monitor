...
const PSMMonitorApp = () => {
  console.log("🚀 PSM Monitor 5.10.23 - FIX: Importa e salva corretamente no Supabase! ✅");
...
// ============================================================================
    ...
    if (Object.keys(data).length > 0) {
      ...
      try {
        ...
        window.salvarSupabaseTimeout = setTimeout(async () => {
          ...
          // V5.10.23: Extrair APENAS dados do ano selecionado para salvar
          const dadosDoAnoAtual = data[selectedYear] || {};
          
          console.log('💾 [DATA] Salvando dados do ano', selectedYear, ':', {
            psms: Object.keys(dadosDoAnoAtual),
            tamanho: JSON.stringify(dadosDoAnoAtual).length
          });
          
          const resultado = await salvarTudoNoSupabase(
            dadosDoAnoAtual,  // V5.10.23: Apenas dados do ano atual
            selectedQuarter,
            selectedYear,
            routeToProvince,
            rotasTestadas[selectedYear] || {},  // V5.10.23: Apenas validações do ano
            rotasValidadas[selectedYear] || {}
          );
    ...
    input.onchange = (event) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          console.log('  Anos existentes:', Object.keys(data));
          console.log('  Ano sendo importado:', selectedYear);
          console.log('  PSMs existentes no ano:', Object.keys(data[selectedYear] || {}));
          ...
          for (let i = 1; i < lines.length; i++) {
            ...
            try {
              ...
              // 13. Criar estrutura se não existir (V5.10.23: COM ANO)
              if (!newData[selectedYear]) newData[selectedYear] = {};
              if (!newData[selectedYear][selectedOperator]) newData[selectedYear][selectedOperator] = {};
              if (!newData[selectedYear][selectedOperator][week]) newData[selectedYear][selectedOperator][week] = {};
              if (!newData[selectedYear][selectedOperator][week][rota]) newData[selectedYear][selectedOperator][week][rota] = {};
              ...
              for (let j = dataStartIndex; j < headers.length && j < values.length; j++) {
                ...
                // 15. Atribuir valor com nome normalizado (V5.10.23: COM ANO)
                newData[selectedYear][selectedOperator][week][rota][statusHeader] = value;
              }
          ...
          console.log('  - Anos após merge:', Object.keys(newData));
          console.log('  - PSMs no ano', selectedYear, ':', Object.keys(newData[selectedYear] || {}));
  ...
  const topRotasCriticas = useMemo(() => {
    ...
    if (top5.length === 0) {
      return [
        { rank: 1, rota: 'Sem dados', value: 0, total: 0, reparadas: 0, semanas: [] },
        { rank: 2, rota: 'Sem dados', value: 0, total: 0, reparadas: 0, semanas: [] },
        { rank: 3, rota: 'Sem dados', value: 0, total: 0, reparadas: 0, semanas: [] },
        { rank: 4, rota: 'Sem dados', value: 0, total: 0, reparadas: 0, semanas: [] },
        { rank: 5, rota: 'Sem dados', value: 0, total: 0, reparadas: 0, semanas: [] }
      ];
    }
  ...
  const rotasNormalizadas = useMemo(() => {
    console.log('🔄 FASE 14: Calculando Rotas Normalizadas (Critérios Corretos)...');
    ...
    routesToProcess.forEach(route => {
      ...
      for (let checkWeek = quarterLimits.start; checkWeek <= quarterLimits.end; checkWeek++) {
        ...
        const foundAnyField = fibrasDependentes !== null || 
                              transporte !== null || 
                              indisponiveis !== null;
        ...
        if (condition1 || condition2 || condition3 || condition4) {
          ...
          console.log(`✅ ${route} normalizada em ${week}:`, {
            ...
            totalIndisponiveisAntes: condition4 ? totalIndisponiveisAntes : undefined,
            reparadasIgualIndisp: condition4 ? (totalReparadas === totalIndisponiveisAntes) : undefined
          });
  ...
  const alertas = useMemo(() => {
    ...
    routesToCheck.forEach(route => {
      ...
      for (let i = quarterConfig[selectedQuarter].start; i <= selectedWeekNum; i++) {
        const w = `W${i}`;
  ...
  const trendData = useMemo(() => {
    ...
    console.log('📊 EVOLUÇÃO TEMPORAL - Filtro Provincial:', {
      ...
      rotasFiltradas: routesToProcess.length,
      quarter: selectedQuarter,
      quarterWeeks: `${quarterWeeks[0]} - ${quarterWeeks[quarterWeeks.length - 1]}`
    });
    ...
    if (weeksAteSelecao.length === 0) {
      console.warn(`⚠️ weeksAteSelecao vazio - selectedWeek ${selectedWeek} não está no quarter ${selectedQuarter}`);
      return [];
    }
    
    console.log(`📅 Semanas no gráfico: ${weeksAteSelecao[0]} até ${selectedWeek} (${weeksAteSelecao.length} semanas)`);
      ...
      routesToProcess.forEach(route => {
        ...
        for (let i = 0; i <= weekIdx; i++) {
          ...
          if (routeData) {
            ...
            const depPass = parseInt(routeData['Dep. de Passagem de Cabo']) || 0;
            const depLic = parseInt(routeData['Dep. de Licença']) || 0;
            const depCut = parseInt(routeData['Dep. de Cutover']) || 0;
            const fibras = parseInt(routeData[`Fibras dependentes da ${selectedOperator}`]) || 0;
            ...
            // Acumular reparadas
            routeLastValues[route].totalReparadas += parseInt(routeData['Total Reparadas']) || 0;
          }
        }
      });
    ...
    console.log(`✓ Evolução Temporal: ${trends.length} semanas até ${selectedWeek}`);
  ...
  const pieChartData = useMemo(() => {
    ...
    const totalSubcategorias = totals.reconhecidas + 
                               totals.depPassagem + totals.depLicenca + 
                               totals.depCutover + totals.fibrasDep;
    
    const innerData = totalSubcategorias > 0 ? [
      { 
        label: 'Reconhecidas', 
        percentage: parseFloat(((totals.reconhecidas / totalSubcategorias) * 100).toFixed(1)),
        value: totals.reconhecidas,
        color: '#06b6d4' 
      },
      { 
        label: 'Dep. Passagem', 
        percentage: parseFloat(((totals.depPassagem / totalSubcategorias) * 100).toFixed(1)),
        value: totals.depPassagem,
        color: '#3b82f6' 
      },
      { 
        label: 'Dep. Licença', 
        percentage: parseFloat(((totals.depLicenca / totalSubcategorias) * 100).toFixed(1)),
        value: totals.depLicenca,
        color: '#f97316' 
      },
      { 
        label: 'Dep. Cutover', 
        percentage: parseFloat(((totals.depCutover / totalSubcategorias) * 100).toFixed(1)),
        value: totals.depCutover,
        color: '#9333ea' 
      },
      { 
        label: `Dep. ${selectedOperator}`, 
        percentage: parseFloat(((totals.fibrasDep / totalSubcategorias) * 100).toFixed(1)),
  ...
  const acompanhamentoData = useMemo(() => {
    ...
    const importedData = Object.entries(justificativas)
      .filter(([key, just]) => {
        ...
        return just.psm === selectedOperator && just.quarter === selectedQuarter;
      })
      .map(([key, just]) => {
